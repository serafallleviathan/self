...
❏ *Levia☆tan* ❏`
    ...
    case 'take':
    case 'colong':
    		if (!isQuotedSticker) return reply('Stiker ae su')
            encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
		    media = await hexa.downloadAndSaveMediaMessage(encmedia)
            anu = args.join(' ').split('|')
	...
	case 'ytsearch':
        	...
        	return await hexa.sendMessage(from, 'gag', MessageType.text, dload)
    		}
    		aramat = aramas.all 
...
case 'ss': case 'ssweb':
      ...
      reply(mess.wait)
          ...
          if (!mek.key.fromMe) return reply('Levia☆tan')
					...
					if (args.length < 1) return reply('kode.e ndi su?')
    ...
    case 'inspect':
            ...
            if (!net) return reply('kudu link https://whatsapp.com/')
            jids = []
